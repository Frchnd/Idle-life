...
function renderMainMenu(state,ui){
  ...
  return `<div class="menu-screen">
    <div class="menu-inner">
      ...
      <div class="build-note">Build T · UI/UX Foundation</div>
    </div>
  </div>`;
}
...
function renderSettings(ui){
  ...
  return `<div class="menu-screen settings-screen">
    <div class="menu-inner settings-inner">
      ...
      <div class="settings-foot">Pengaturan tampilan disimpan terpisah dari save hidupmu.</div>
    </div>
  </div>`;
}
...
function renderLife(state,ui){
  ...
  return `<section id="life" class="page-stack">
    ...
    <div class="card secondary-card">
      <div class="card-head"><div><div class="section-title">Rutinitas</div><div class="subline">${state.flags.routineUnlocked?(state.routine.enabled?'Aktif · hal repetitif berjalan otomatis':'Tersedia, tapi belum aktif'):'Terbuka setelah kamu punya ritme kerja.'}</div></div><button class="btn mini center" data-ui="toggle-routine" ${state.flags.routineUnlocked?'':'disabled'}>${state.routine.enabled?'Matikan':'Aktifkan'}</button></div>
    ...
    <div class="stat-strip"><div><span>Uang</span><b>${compactMoney(state.player.money)}</b></div><div><span>Kondisi</span><b>${esc(condition.label)}</b></div><div><span>Arah</span><b>${esc(trajectoryLabel(state))}</b></div></div>
  </section>`;
}
...
function renderWorld(state){
  ...
  return `<section id="world" class="page-stack">
    <div class="card">
      <div class="card-head"><div><div class="eyebrow">DUNIA</div><div class="section-title large">Kondisi ${esc(world.phase)}</div><div class="subline">Simulasi bergerak setiap 7 hari game.</div></div><span class="pill">Minggu ${state.world?.week||0}</span></div>
      <div class="metric-grid"><div class="metric"><span>Pasar kerja</span><b>${esc(world.jobMarket)}</b></div><div class="metric"><span>Biaya hidup</span><b>${esc(world.costTrend)}</b></div><div class="metric"><span>Bengkel</span><b>${esc(world.mechanics)}</b></div><div class="metric"><span>Retail</span><b>${esc(world.retail)}</b></div><div class="metric"><span>Teknologi</span><b>${esc(world.technology)}</b></div></div>
      ${world.news.length?`<div class="signal-list">${world.news.map(x=>`<div class="signal">${esc(x)}</div>`).join('')}</div>`:''}
    </div>
    ...
    <div class="card"><div class="eyebrow">TEMPAT KERJA</div><div class="section-title">Kondisi perusahaan</div><div class="list clean-list">${Object.values(world.workplaces||{}).map(c=>`<div class="list-row"><div><b>${esc(c.name)}</b><div class="subline">Arus usaha ${esc(companyCashflowLabel(c.margin||0))} · ${c.headcount||'-'} orang</div></div><div class="align-right"><b>${esc(c.label)}</b><div class="subline">Risiko ${esc(employmentRiskLabel(c))}</div></div></div>`).join('')}</div></div>

    <div class="card"><div class="card-head"><div><div class="eyebrow">PELUANG</div><div class="section-title">Yang sedang terbuka</div></div><span class="pill">${activeOpps.length}</span></div><div class="list clean-list">${activeOpps.length?activeOpps.map(o=>`<div class="list-row single"><div><b>${esc(o.name)}</b><div class="subline">${esc(o.summary)}</div></div></div>`).join(''):'<div class="empty">Belum ada peluang penting.</div>'}</div></div>
    ...
    <div class="card"><div class="eyebrow">TERBARU</div><div class="section-title">Perubahan dunia</div><div class="timeline">${state.recent.length?state.recent.slice(0,8).map(x=>`<div class="timeline-item">${esc(x)}</div>`).join(''):'<div class="empty">Belum ada hal penting.</div>'}</div></div>
  </section>`;
}
...
function renderYou(state){
  ...
  return `<section id="you" class="page-stack">
    <div class="card profile-card"><div class="eyebrow">JEJAK HIDUP</div><div class="section-title hero-title">${esc(profile.title)}</div><div class="profile-summary">${esc(profile.summary)}</div>${state.flags.verticalSliceComplete?'<span class="pill space-top">Bab 1 terbentuk</span>':''}</div>
    ...
    <div class="card"><div class="eyebrow">KEADAAN HIDUP</div><div class="key-row"><span>Arah</span><b>${esc(lifeDirection(state))}</b></div><div class="key-row"><span>Strategi</span><b>${esc(trajectoryLabel(state))}</b></div><div class="key-row"><span>Tempat tinggal</span><b>${esc(housingLabel(state))}</b></div><div class="key-row"><span>Keuangan</span><b>${esc(finance.label)}</b></div><div class="subline space-top">Biaya hidup ${money(state.economy.livingCost)}/bulan</div>${statuses.length?`<div class="tag-cloud">${statuses.map(x=>`<span class="tag">${esc(x)}</span>`).join('')}</div>`:''}</div>
...
function renderGameHeader(state){
  ...
  return `<header class="game-header">
    ...
    <div class="game-header-right"><div class="header-money">${money(state.player.money)}</div><div class="header-condition">${esc(condition.label)}</div></div>
    <button class="icon-btn menu-btn" data-ui="open-menu" aria-label="Buka menu">•••</button>
  </header>`;
}

function renderGame(root,state,ui){
  root.innerHTML=`<div class="game-shell">${renderGameHeader(state)}<main class="game-main">${ui.tab==='life'?renderLife(state,ui):ui.tab==='world'?renderWorld(state):renderYou(state)}</main><nav class="tabs" aria-label="Navigasi utama"><button class="tab ${ui.tab==='life'?'active':''}" data-tab="life"><span>HIDUP</span></button><button class="tab ${ui.tab==='world'?'active':''}" data-tab="world"><span>DUNIA</span></button><button class="tab ${ui.tab==='you'?'active':''}" data-tab="you"><span>KAMU</span></button></nav></div>`;
}

function render(root,state,ui){
  if(ui.screen==='settings'){root.innerHTML=renderSettings(ui);return;}
  if(ui.screen==='menu'){root.innerHTML=renderMainMenu(state,ui);return;}
