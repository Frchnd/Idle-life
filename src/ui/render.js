...
function renderYou(state){
  ...
  ${state.business?.active?`<div class="divider"></div><div class="section-title">Usaha Kecil</div><div class="item" style="margin-top:10px"><div class="row"><b>${esc(state.business.name||'Usaha Kecil')}</b><span class="pill">${esc(businessHealthLabel(state))}</span></div><div class="muted small" style="margin-top:6px">Ditangani minggu ini: ${state.business.servedClients||state.business.clients||0}/${state.business.capacity||2} kapasitas · pelanggan tetap: ${state.business.retainedClients||0}</div><div class="muted small" style="margin-top:4px">Permintaan terlewat: ${state.business.missedDemand||0} · peralatan: ${esc(businessEquipmentLabel(state))}</div><div class="muted small" style="margin-top:4px">Profit minggu terakhir: ${money(state.business.lastWeeklyProfit||0)} · total: ${money(state.business.totalProfit||0)}</div></div>`:''}
