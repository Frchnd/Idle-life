...
export function executeActivity(state,id){
  if(id==='job_search'){
    state.time.totalHours+=6; state.player.fatigue=Math.min(100,state.player.fatigue+8); state.career.jobSearchCount++;
  ...
  if(id==='study'){
    ...
    state.player.money-=20000; state.time.totalHours+=4; state.player.fatigue=Math.min(100,state.player.fatigue+10);
    state.skills.learning+=10; state.skills.technology+=Math.round(10*xpMod(state));
  ...
  if(id==='family'){
    state.time.totalHours+=4; state.player.fatigue=Math.min(100,state.player.fatigue+7); state.relationships.family+=4; state.skills.social+=3;
    return 'Kamu membantu keluarga dan menghabiskan waktu bersama mereka.';
  }
  if(id==='rian'){
    state.time.totalHours+=3; state.player.fatigue=Math.min(100,state.player.fatigue+5); state.relationships.rian=Math.min(49,state.relationships.rian+3); state.skills.social+=6;
    return 'Kamu menghabiskan waktu bersama Rian. Hubungan kalian tetap hangat.';
  }
  if(id==='rest'){
    state.time.totalHours+=8; state.player.fatigue=Math.max(0,state.player.fatigue-45);
    return 'Kamu beristirahat dan memulihkan kondisi.';
  }
  if(id==='work'){
    const job=JOBS[state.player.job];
    if(!job) return {error:'Pekerjaan aktif tidak ditemukan.'};
    state.time.totalHours+=job.duration; state.player.money+=job.salary; state.player.fatigue=Math.min(100,state.player.fatigue+job.fatigue);
    ...
    return `Kerja selesai · +Rp${job.salary.toLocaleString('id-ID')} · kemampuan ${job.skill==='mechanics'?'Mekanik':'Sosial'} meningkat.`;
  }
